    ...
    <div class="main">
      ...
      <div class="rank">
          ...
          <div class="contenttitle">
            <h1>月間ランキング</h1>
            <div class="languege">
              <form action="../../trends/monthly.php" method="post">
          ...
          if ($country == "JP") {
            ...
            for ($i = 0; $i < 10; $i++) {
              ...
              $res = $pdo->query("SELECT title_name FROM news WHERE title_day >  (NOW() - INTERVAL 1 MONTH) ORDER BY count DESC LIMIT  $i ,1");
              $res->execute();
              $user = $res->fetch();
          ...
          ?>
              ...
              <?php
              $hoge = '<a href="../../trends/monthly.php/' . ($page) . '" class="btn btn-gradient">' . $page . '</a>';
              ?>
              <div class="trend">
                <?php print $hoge; ?>
              </div>
            ...
            for ($i = 0; $i < 10; $i++) {
              ...
              $res = $pdo->query("SELECT title_name FROM news_us WHERE title_day >  (NOW() - INTERVAL 1 MONTH) ORDER BY count DESC LIMIT  $i ,1");       //DBからデータを取ってくる
              $res->execute();
              $user = $res->fetch();
              $page = $user['title_name'];
            ?>
            ...
            for ($i = 0; $i < 10; $i++) {
              ...
              $res = $pdo->query("SELECT title_name FROM news_ind WHERE title_day >  (NOW() - INTERVAL 1 MONTH) ORDER BY count DESC LIMIT  $i ,1");       //DBからデータを取ってくる
              $res->execute();
              $user = $res->fetch();
              $page = $user['title_name'];
            ?>
